...
const SOCIAL_ICONS = [
  { href: 'https://instagram.com/nammakuppai', title: 'Instagram', path: 'M12 0C8.74 0 8.333.015 7.053.072 5.775.132 4.905.333 4.14.63c-.789.306-1.459.717-2.126 1.384S.935 3.35.63 4.14C.333 4.905.131 5.775.072 7.053.012 8.333 0 8.74 0 12s.015 3.667.072 4.947c.06 1.277.261 2.148.558 2.913.306.788.717 1.459 1.384 2.126.667.666 1.336 1.079 2.126 1.384.766.296 1.636.499 2.913.558C8.333 23.988 8.74 24 12 24s3.667-.015 4.947-.072c1.277-.06 2.148-.262 2.913-.558.788-.306 1.459-.718 2.126-1.384.666-.667 1.079-1.335 1.384-2.126.296-.765.499-1.636.558-2.913.06-1.28.072-1.687.072-4.947s-.015-3.667-.072-4.947c-.06-1.277-.262-2.149-.558-2.913-.306-.789-.718-1.459-1.384-2.126C21.319 1.347 20.651.935 19.86.63c-.765-.297-1.636-.499-2.913-.558C15.667.012 15.26 0 12 0zm0 2.16c3.203 0 3.585.016 4.85.071 1.17.055 1.805.249 2.227.415.562.217.96.477 1.382.896.419.42.679.819.896 1.381.164.422.36 1.057.413 2.227.057 1.266.07 1.646.07 4.85s-.015 3.585-.074 4.85c-.061 1.17-.256 1.805-.421 2.227-.224.562-.479.96-.899 1.382-.419.419-.824.679-1.38.896-.42.164-1.065.36-2.235.413-1.274.057-1.649.07-4.859.07-3.211 0-3.586-.015-4.859-.074-1.171-.061-1.816-.256-2.236-.421-.569-.224-.96-.479-1.379-.899-.421-.419-.69-.824-.9-1.38-.165-.42-.359-1.065-.42-2.235-.045-1.26-.061-1.649-.061-4.844 0-3.196.016-3.586.061-4.861.061-1.17.255-1.814.42-2.234.21-.57.479-.96.9-1.381.419-.419.81-.689 1.379-.898.42-.166 1.051-.361 2.221-.421 1.275-.045 1.65-.06 4.859-.06l.045.03zm0 3.678a6.162 6.162 0 100 12.324 6.162 6.162 0 100-12.324zM12 16c-2.21 0-4-1.79-4-4s1.79-4 4-4 4 1.79 4 4-1.79 4-4 4zm7.846-10.405a1.441 1.441 0 11-2.88 0 1.441 1.441 0 012.88 0z' },
  { href: 'https://x.com/nammakuppai', title: 'X', path: 'M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.747l7.73-8.835L1.254 2.25H8.08l4.253 5.622zm-1.161 17.52h1.833L7.084 4.126H5.117z' },
  { href: 'https://www.linkedin.com/company/namma-kuppai/', title: 'LinkedIn', path: 'M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z' },
  { href: 'https://t.me/nammakuppai', title: 'Telegram', path: 'M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z' },
]
...
function AnimatedSocialIcon() {
  ...
  useEffect(() => {
    const cycle = setInterval(() => {
      setVisible(false)
      setTimeout(() => {
        setCurrent(prev => (prev + 1) % SOCIAL_ICONS.length)
        setVisible(true)
      }, 400)
    }, 2500)
  ...
  const icon = SOCIAL_ICONS[current]
...
function Home({ lang, setLang }) {
  ...
  async function submitCleaned() {
    ...
    try {
      ...
      const { error: uploadError } = await supabase.storage.from('reports').upload(fileName, cleanedPhoto)
      if (uploadError) throw uploadError
      const { data: urlData } = supabase.storage.from('reports').getPublicUrl(fileName)
      const verifiedPhotoUrl = urlData.publicUrl
      const { error: updateError } = await supabase
        .from('reports')
        .update({ status: 'resolved', verified_photo_url: verifiedPhotoUrl })
        .eq('id', selectedReport.id)
      if (updateError) throw updateError
      const updatedReport = { ...selectedReport, status: 'resolved', verified_photo_url: verifiedPhotoUrl }
      setReports(reports.map(r => r.id === selectedReport.id ? updatedReport : r))
      setSelectedReport(updatedReport)
      setCleanedSuccess(true)
      setTimeout(() => { setShowCleanedModal(false); setCleanedSuccess(false); setSelectedReport(updatedReport) }, 2500)
    } catch (error) {
    ...
    <div className="app-container">
      ...
      <div className="header">
        ...
        <div className="header-right">
          ...
          >
            <span className={lang === 'ta' ? 'lang-active' : 'lang-inactive'}>த</span>
          ...
          <div className="map-container">
            ...
            <div className="map-stats">
              ...
              <div className="map-stat">
                ...
                <span className="map-stat-label">{t.active}</span>
              </div>
              ...
              reports.map((report, index) => (
                <div key={report.id} className="list-card" onClick={() => setSelectedReport(reports.find(r => r.id === report.id) || report)}>
                  ...
                  <div className="list-info">
                    ...
                    {report.mla_constituency && <p className="list-meta">MLA: {report.mla_constituency}</p>}
                    {report.mp_constituency && <p className="list-meta">MP: {report.mp_constituency}</p>}
                    <p className="list-time">{timeAgo(report.created_at)}</p>
                  </div>
                  <div className="list-right">
                    <span className="list-severity" style={{ background: severityColors[report.severity]?.bg, color: severityColors[report.severity]?.color }}>
                      {severityColors[report.severity]?.label || report.severity}
                    </span>
                    {report.status === 'resolved' && <span className="list-resolved">✅ Resolved</span>}
                    {report.photo_url && <img src={report.photo_url} alt="Report" className="list-thumb" />}
                  </div>
                </div>
              ))
            )}
          ...
          <div className="detail-modal" onClick={e => e.stopPropagation()}>
            <div className="detail-header">
              <div className="detail-badges">
                <span className="detail-severity-badge" style={{ background: severityColors[selectedReport.severity]?.bg, color: severityColors[selectedReport.severity]?.color }}>
            ...
            <h2 className="detail-ward-title">{selectedReport.ward_name || 'Unknown Ward'}</h2>
            <p className="detail-ward-sub">Ward #{selectedReport.ward_number || '?'} · {selectedReport.corporation || 'GCC'} · {timeAgo(selectedReport.created_at)}</p>
            ...
            <div className="detail-stats-row">
              ...
              <div className="detail-stat">
                <span className="detail-stat-num">{Math.floor((new Date() - new Date(selectedReport.created_at)) / 86400000) || 0}</span>
                <span className="detail-stat-label">Days open</span>
              </div>
              <div className="detail-stat-divider" />
              <div className="detail-stat">
                <span className="detail-stat-num" style={{fontSize:'12px'}}>{selectedReport.zone || 'GCC'}</span>
            ...
            <div className="detail-accountability">
              ...
              <div className="detail-ward-badge">Your Ward · #{selectedReport.ward_number || '?'}</div>
              <div className="detail-politicians">
                ...
                <div className="detail-politician">
                  ...
                  <div className="detail-politician-info">
                    ...
                    <span className="detail-politician-name">{selectedReport.mla_name || 'Name pending'}</span>
                  </div>
                </div>
                <div className="detail-politician">
                  <div className="detail-politician-icon">🇮🇳</div>
                  <div className="detail-politician-info">
                    ...
                    <span className="detail-politician-name">{selectedReport.mp_name || 'Name pending'}</span>
                  </div>
                </div>
              </div>
            </div>
            <div className="detail-actions">
              <button className="detail-whatsapp-share-btn" onClick={() => shareOnWhatsApp(selectedReport)}>
              ...
              <a href="https://gccservices.chennaicorporation.gov.in/pgr/home" target="_blank" className="detail-complaint-btn">
          ...
          <div className="form-container" onClick={e => e.stopPropagation()}>
            ...
            {cleanedSuccess ? (
              <div className="success-message">
                {lang === 'ta' ? '🎉 நன்றி! சிங்கார சென்னைக்கு உதவியதற்கு நன்றி!' : '🎉 Thank you! Marked as cleaned.\nFor the Singara Chennai we grew up loving 💛'}
              </div>
            ) : (
              <>
                <p style={{fontSize:'14px', color:'#555', marginBottom:'16px', lineHeight:'1.5'}}>
                  {lang === 'ta' ? 'சுத்தம் செய்யப்பட்ட இடத்தின் படம் எடுத்து பதிவேற்றவும்' : 'Upload a photo showing the area is now clean.'}
                </p>
                <div className="photo-upload">
                  {cleanedPhotoPreview ? (
                    <div style={{position:'relative'}}>
                      <img src={cleanedPhotoPreview} alt="Clean preview" className="photo-preview" />
                      <button onClick={() => { setCleanedPhoto(null); setCleanedPhotoPreview(null) }} style={{position:'absolute',top:'8px',right:'8px',background:'rgba(0,0,0,0.5)',color:'#fff',border:'none',borderRadius:'50%',width:'24px',height:'24px',cursor:'pointer',fontSize:'12px'}}>✕</button>
                    </div>
                  ) : (
                ...
                <button className="submit-btn" onClick={submitCleaned} disabled={cleanedLoading || !cleanedPhoto}
                  style={{ opacity: cleanedLoading || !cleanedPhoto ? 0.45 : 1, cursor: cleanedLoading || !cleanedPhoto ? 'not-allowed' : 'pointer', background: '#16a34a' }}>
                  {cleanedLoading ? (lang === 'ta' ? 'சமர்ப்பிக்கிறது...' : 'Submitting...') : (lang === 'ta' ? '✅ சுத்தம் உறுதிப்படுத்து' : '✅ Confirm Cleaned')}
                </button>
              </>
            )}
          </div>
        ...
        <div className="form-overlay" onClick={() => setShowQR(false)}>
          <div className="qr-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="qr-code-box">
              <img src="https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=https://nammakuppai.in" alt="QR Code" width="180" height="180" />
            </div>
      ...
      <div className="footer">
        <div className="social-icons">
          <a href="https://instagram.com/nammakuppai" target="_blank" className="social-icon footer-social" title="Instagram">
          ...
          <a href="https://x.com/nammakuppai" target="_blank" className="social-icon footer-social" title="X">
            <svg viewBox="0 0 24 24" width="15" height="15" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.747l7.73-8.835L1.254 2.25H8.08l4.253 5.622zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
          </a>
          <a href="https://www.linkedin.com/company/namma-kuppai/" target="_blank" className="social-icon footer-social" title="LinkedIn">
          ...
          <a href="https://t.me/nammakuppai" target="_blank" className="social-icon footer-social" title="Telegram">
        ...
        <span className="footer-copyright">© Namma Kuppai 2026</span>
      </div>
